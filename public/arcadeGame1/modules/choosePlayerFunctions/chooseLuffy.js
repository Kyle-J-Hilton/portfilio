...
import * as Explode from "../assets/explode.js";
 ...
 export function chooseLuffy() {
    ...
    let count = 0;
    let lcount = 0;
    let player = fighter.luffy;
    let enemy = fighter.naruto;
    ...
    background.image.src = "/arcadeGame1/modules/img/hiddenleaf_background.png";
    //start timer
    decreaseTimer();
    //start animations
    function animate() {
    ...
    window.addEventListener("keydown", (event) => {
        ...
        case "m":
            //player 2 throw attack
            fighter.naruto.islongRange = true;
            if (fighter.naruto.position.x >= player.position.x) {
                fighter.naruto.longRangeLeft();
            setTimeout(() => {
                jutsus.push(
                new Jutsu({
                    position: {
                    x: fighter.naruto.position.x - 50,
                    y: fighter.naruto.position.y,
                    },
                    velocity: {
                    x: -9,
                    y: -3,
